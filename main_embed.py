...
INTENT_MIN_SIM = 0.38
INTENT_MARGIN = 0.03
INTENT_FOLLOWUP_SIM = 0.24
BRAND_MIN_SIM = 0.36
BRAND_MARGIN = 0.02
BRAND_KB_SUPPRESS = 0.68
MULTI_QUERY_MIN_SIM = 0.36
MULTI_QUERY_MARGIN = 0.02
SELECTION_MIN_SIM = 0.36
...
def init_db():
    ...
    try:
        ...
        pass
    try:
        conn.execute("ALTER TABLE user_state ADD COLUMN last_products_json TEXT")
    except sqlite3.OperationalError:
        pass
    conn.commit()
    conn.close()

def get_user_state(user_id: int) -> tuple[Optional[str], Optional[str], bool, List[str]]:
    conn = get_db()
    row = conn.execute(
        "SELECT last_product, last_intent, last_product_known, last_products_json FROM user_state WHERE user_id=?",
        (user_id,)
    ).fetchone()
    conn.close()
    if not row:
    ...
    if last_product and last_product.startswith("__UNK__"):
        last_product = last_product[len("__UNK__"):]
        known = False
    products_list: List[str] = []
    try:
        if row["last_products_json"]:
            products_list = json.loads(row["last_products_json"])
    except Exception:
        products_list = []
    return last_product, last_intent, known, products_list
...
def set_user_state(
    ...
    conn.execute(
        """
        INSERT INTO user_state (user_id, last_product, last_intent, last_product_known, last_products_json, updated_at)
        ...
        ON CONFLICT(user_id) DO UPDATE SET
            ...
            last_products_json=COALESCE(excluded.last_products_json, user_state.last_products_json),
        ...
        (
            ...
            product_name,
            intent,
            None if product_known is None else (1 if product_known else 0),
            json.dumps(products_list) if products_list is not None else None,
        )
    )
    conn.commit()
    conn.close()
...
class IntentResolver:
    def __init__(self, rag_engine: "RAGEngine"):
        self.rag = rag_engine
        self._intent_anchors: dict[str, List[str]] = {
            "availability": [
                "এই পণ্যটি পাওয়া যায় কি?",
                "আপনাদের দোকানে আছে কি?",
                "স্টকে আছে কি?",
                "available কি?",
            ],
            "price": [
        ...
        self._intent_vecs: dict[str, np.ndarray] = {
            k: self.rag._model.encode(v, normalize_embeddings=True).astype(np.float32)
            for k, v in self._intent_anchors.items()
        }

    def score_intent(self, message: str, intent: str) -> float:
        vecs = self._intent_vecs.get(intent)
        if vecs is None:
            return 0.0
        qvec = self.rag._get_query_embedding(message)
        return float(np.max(vecs @ qvec))
...
class BrandResolver:
    def __init__(self, rag_engine: "RAGEngine"):
        self.rag = rag_engine
        self._brand_vecs = self.rag._model.encode(
            [
                "আমি নির্দিষ্ট ব্র্যান্ডের পণ্য চাই।",
                "আমি একটি ব্র্যান্ডের নাম বলছি।",
                "আমি ব্র্যান্ড-নির্দিষ্ট পণ্য খুঁজছি।",
                "BrandX জুতা",
                "CompanyY ফোন",
                "Zeta ব্র্যান্ডের ঘড়ি",
                "ABC ব্র্যান্ডের শার্ট",
            ],
            normalize_embeddings=True,
        ).astype(np.float32)
        self._generic_vecs = self.rag._model.encode(
            [
                "আমি সাধারণ পণ্য চাই।",
                "আমি ব্র্যান্ড না বলে শুধু পণ্যের ধরন বলছি।",
                "আমি পণ্যের ধরন বা ক্যাটাগরি বলছি।",
                "জুতা",
                "ফোন",
                "নুডুলস",
            ],
    ...
    def is_brand_specific(self, message: str) -> bool:
        qvec = self.rag._get_query_embedding(message)
        brand_score = float(np.max(self._brand_vecs @ qvec))
        generic_score = float(np.max(self._generic_vecs @ qvec))
        # Suppress brand classification when query is already close to a KB product name
        if self.rag.product_name_embeddings is not None:
            kb_sim = float(np.max(self.rag.product_name_embeddings @ qvec))
            if kb_sim >= BRAND_KB_SUPPRESS:
                return False
        return brand_score >= BRAND_MIN_SIM and (brand_score - generic_score) >= BRAND_MARGIN


class MultiQueryResolver:
    def __init__(self, rag_engine: "RAGEngine"):
        self.rag = rag_engine
        self._multi_vecs = self.rag._model.encode(
            [
                "আমি একাধিক পণ্যের বিষয়ে জানতে চাই।",
                "আমি একসাথে দুইটি আলাদা পণ্যের কথা বলছি।",
                "চা এবং কফি আছে কি?",
                "জুতা ও ঘড়ি আছে কি?",
            ],
            normalize_embeddings=True,
        ).astype(np.float32)
        self._single_vecs = self.rag._model.encode(
            [
                "আমি একটি পণ্যের বিষয়ে জানতে চাই।",
                "আমি একটি পণ্য সম্পর্কে বলছি।",
                "নুডুলস আছে কি?",
                "চা আছে কি?",
            ],
            normalize_embeddings=True,
    ...
    def is_multi_query(self, message: str) -> bool:
        qvec = self.rag._get_query_embedding(message)
        multi_score = float(np.max(self._multi_vecs @ qvec))
        single_score = float(np.max(self._single_vecs @ qvec))
        return multi_score >= MULTI_QUERY_MIN_SIM and (multi_score - single_score) >= MULTI_QUERY_MARGIN


class SelectionResolver:
    def __init__(self, rag_engine: "RAGEngine"):
        self.rag = rag_engine
        self._select_vecs = self.rag._model.encode(
            [
                "আমি একটি নির্দিষ্ট পণ্যের নাম বলছি।",
                "আমি একটি ভ্যারিয়েন্ট নির্বাচন করছি।",
                "ম্যাগি নুডুলস",
                "আরামদায়ক জুতা",
            ],
            normalize_embeddings=True,
        ).astype(np.float32)
        self._question_vecs = self.rag._model.encode(
            [
                "রিভিউ কত?",
                "দাম কত?",
                "ডেলিভারি কেমন?",
    ...
    def is_selection(self, message: str) -> bool:
        qvec = self.rag._get_query_embedding(message)
        sel_score = float(np.max(self._select_vecs @ qvec))
        q_score = float(np.max(self._question_vecs @ qvec))
        return sel_score >= SELECTION_MIN_SIM and (sel_score - q_score) >= SELECTION_MARGIN
...
def _build_multi_product_answer(
    ...
    for term in terms:
        ...
        if term in rag.product_token_map:
            ...
            cand = rag.matched_product_names(term)
            if cand:
                product_name = cand[0]
        if product_name:
...
intent_resolver = IntentResolver(rag)
...
def chat(req: ChatRequest, current_user=Depends(get_current_user)):
    ...
    brand_specific = brand_resolver.is_brand_specific(msg_norm)
    product_in_msg = len(msg_matched_names) > 0 or semantic_hit_domain or brand_specific
    msg_has_own_context = product_in_msg
    multi_product = len(msg_matched_names) > 1 and multi_query_resolver.is_multi_query(msg_norm)
    ...
    if product_in_msg:
        query_domain = "store"
    if primary_intent in {"availability", "price", "discount", "warranty", "delivery", "payment", "rating", "features", "popularity"}:
        query_domain = "store"
    ...
    if query_domain == "store":
        ...
        product_token_hit = product_in_query

    # Brand-specific query without exact KB match -> treat as unknown product
    brand_unknown = brand_specific and (not matched_names) and (not semantic_product_hit)
    if brand_unknown:
        semantic_product_hit = False
        semantic_product_score = 0.0
        semantic_product_name = None
    ...
    # Decide which product(s) to store as last_product (embedding-based)
    state_products: List[str] = []
    state_known = False
    if msg_matched_names:
        state_products = _clean_product_names(msg_matched_names, limit=5)
        state_known = True
    elif brand_unknown:
        state_products = [msg_norm]
        state_known = False
    elif semantic_product_hit and semantic_product_name:
    ...
    if selection_resolver.is_selection(msg_norm) and last_intent:
        primary_intent = last_intent

    followup_intent = primary_intent is not None

    # Derived intent flags
    is_price_query = primary_intent == "price"
    is_discount_query = primary_intent == "discount"
    is_availability_query = primary_intent == "availability"
    is_feature_query = primary_intent == "features"
    is_warranty_query = primary_intent == "warranty"
    is_delivery_query = primary_intent == "delivery"
    is_payment_query = primary_intent == "payment"
    is_rating_query = primary_intent == "rating"
    is_popularity_query = primary_intent == "popularity"

    # General-chat handling (embedding-based)
    if (query_domain == "general") and (not product_in_msg) and (primary_intent in {None, "general", "greeting"}):
        if not groq_client:
    ...
    # If last state has multiple variants, require explicit selection for specific intents
    if (
        not msg_has_own_context
        and followup_intent
        and last_products_list
        and len(last_products_list) > 1
        and primary_intent in {"rating", "price", "warranty", "delivery", "features", "discount"}
        and not selection_resolver.is_selection(msg_norm)
    ):
        variants_str = _join_products(_clean_product_names(last_products_list, limit=5))
        ...
        set_user_state(
            ...
            product_known=last_product_known,
            products_list=last_products_list,
        )
        conn.execute("INSERT INTO chat_history (user_id, role, content) VALUES (?,?,?)",
                     (user_id, "user", req.message))
        conn.execute("INSERT INTO chat_history (user_id, role, content) VALUES (?,?,?)",
    ...
    if (
        ...
        # If multiple variants were stored, require user clarification for specific intents
        if last_products_list and len(last_products_list) > 1 and primary_intent in {"rating", "price", "warranty", "delivery", "features", "discount"}:
            variants_str = _join_products(_clean_product_names(last_products_list, limit=5))
            answer = f"একাধিক ভ্যারিয়েন্ট আছে: {variants_str}। আপনি কোনটির {_intent_label_bn(primary_intent)} জানতে চান?"
            decision = "variant_clarify"
            generation_ms = 0.0
    ...
    if multi_targets and primary_intent:
        ...
        if multi_answer:
            ...
            generation_ms = 0.0
            matched_names = []
    elif brand_unknown and primary_intent:
        answer = "দুঃখিত, এই পণ্যটি আমাদের স্টোরে পাওয়া যায় না।"
        decision = "brand_unknown"
        generation_ms = 0.0
...
--- End of Knowledge Base ---"""
    ...
    if state_products or primary_intent:
        ...
        set_user_state(
            ...
            products_list=state_products if state_products else None,
        )
    conn.execute("INSERT INTO chat_history (user_id, role, content) VALUES (?,?,?)",
                 (user_id, "user", req.message))
    conn.execute("INSERT INTO chat_history (user_id, role, content) VALUES (?,?,?)",
    ...
    print(
        ...
        f"brand_specific={brand_specific}",
        f"brand_unknown={brand_unknown}",
        f"multi_query={multi_product}",
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main_embed:app", host="0.0.0.0", port=8000, reload=False)
